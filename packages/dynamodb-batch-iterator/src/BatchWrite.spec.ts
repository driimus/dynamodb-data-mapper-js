    ...
    describe.each([true, false])('should', (asyncInput) => {
        ...
        it(`partition write batches into requests with ${MAX_WRITE_BATCH_SIZE} or fewer items`, async () => {
            ...
            for (let i = 0; i < 80; i++) {
                ...
                expected[Math.floor(i / MAX_WRITE_BATCH_SIZE)][0].RequestItems[
                    table
                ].push(req);
            }
            ...
            for await (const [tableName, req] of new BatchWrite(
            ...
            expect(calls.length).toBe(
                Math.ceil(writes.length / MAX_WRITE_BATCH_SIZE)
            );
        ...
        it('retry unprocessed items', async () => {
            ...
            promiseFunc.mockImplementation(async () => {
                ...
                for (const tableName of Object.keys(RequestItems)) {
                    for (const { DeleteRequest, PutRequest } of RequestItems[
                        tableName
                    ]) {
                        const item = DeleteRequest
                            ? DeleteRequest.Key
                            : PutRequest?.Item;
                        if (unprocessed.has((item as any).fizz.N)) {
                            ...
                            response.UnprocessedItems[tableName].push(
                                unprocessed.get((item as any).fizz.N) as object
                            );
                            unprocessed.delete((item as any).fizz.N);
                        }
                    }
                }
