	...
	describe('#marshallValue', () => {
		...
		describe('maps', () => {
			...
			it('should omit keys whose values are serialized as undefined', () => {
				...
				expect(
					marshaller.marshallValue(new Map([['a', undefined]])),
				).toEqual({M: {}});
			});
		...
		describe('sets', () => {
			...
			describe('number sets', () => {
				...
				it('should convert BigIntSet objects into NumberSetAttributeValues', () => {
					expect(
						new Marshaller().marshallValue(
							new Set([BigInt('1'), BigInt('2'), BigInt('3')]),
						),
					).toEqual({NS: ['1', '2', '3']});
				});
			});
		...
		describe('symbols', () => {
			it('should omit symbols when the onInvalid option is set to "omit"', () => {
				expect(
					new Marshaller({onInvalid: 'omit'}).marshallValue(
						Symbol.iterator,
					),
				).toBeUndefined();
			});
	...
	describe('#unmarshallValue', () => {
		...
		describe('strings', () => {
			it('should convert StringAttributeValues to strings', () => {
				expect(marshaller.unmarshallValue({S: 'string'})).toEqual(
					'string',
				);
			});
		});
		...
		describe('binary values', () => {
			it('should convert BinaryAttributeValues to binary values', () => {
				expect(
					marshaller.unmarshallValue({B: new Uint8Array(1)}),
		...
		describe('null', () => {
			it('should convert NullAttributeValues to null', () => {
				expect(marshaller.unmarshallValue({NULL: true})).toEqual(
					null,
				);
			});
		});
		...
		describe('boolean', () => {
			it('should convert BooleanAttributeValues to booleans', () => {
				expect(marshaller.unmarshallValue({BOOL: true})).toEqual(
					true,
				);
				expect(marshaller.unmarshallValue({BOOL: false})).toEqual(
					false,
				);
			});
		});
		...
		describe('string sets', () => {
			it('should convert StringSetAttributeValues into sets with strings', () => {
				expect(
					marshaller.unmarshallValue({SS: ['a', 'b', 'c']}),
		...
		describe('number sets', () => {
			...
			it('should convert NumberSetAttributeValues into sets with numbers when unwrapNumbers is true', () => {
				const marshaller = new Marshaller({unwrapNumbers: true});
				expect(
					marshaller.unmarshallValue({NS: ['1', '2', '3']}),
		...
		describe('binary sets', () => {
			it('should convert BinarySetAttributeValues into sets with binary strings', () => {
				expect(
					...
					new BinarySet([new Uint8Array(1), new Uint8Array(2)]),
				);
			});
		});
