...
describe('Marshaller', () => {
  describe('#marshallItem', () => {
    ...
    it('should return an empty attribute map when provided invalid input and the onInvalid option is set to "omit"', () => {
  ...
  describe('#marshallValue', () => {
    describe('strings', () => {
      it('should convert strings to StringAttributeValues', () => {
        expect(new Marshaller().marshallValue('string')).toEqual({
          S: 'string',
        });
      });

      it('should convert empty strings to null when onEmpty option set to "nullify"', () => {
        expect(new Marshaller({ onEmpty: 'nullify' }).marshallValue('')).toEqual({ NULL: true });
      });

      it('should remove empty strings when onEmpty option set to "omit"', () => {
        expect(new Marshaller({ onEmpty: 'omit' }).marshallValue('')).toBeUndefined();
      });

      it('should convert empty strings to StringAttributeValues otherwise', () => {
        expect(new Marshaller().marshallValue('')).toEqual({ S: '' });
      });
    });
    ...
    describe('binary values', () => {
      it('should convert binary values to BinaryAttributeValues', () => {
        ...
        expect(new Marshaller().marshallValue(bin)).toEqual({ B: bin });
      });

      it.skip('should convert empty binary values to null when onEmpty option set to "nullify"', () => {
        expect(new Marshaller({ onEmpty: 'nullify' }).marshallValue(new Uint8Array(0))).toEqual({
          NULL: true,
        });
      });

      it.skip('should omit empty binary values when onEmpty option set to "omit"', () => {
        expect(
          new Marshaller({ onEmpty: 'omit' }).marshallValue(new Uint8Array(0))
        ).toBeUndefined();
      });

      it('should convert empty binary values to null when onEmpty option set to "nullify"', () => {
        expect(new Marshaller().marshallValue(new Uint8Array(0))).toEqual({ B: new Uint8Array(0) });
      });
    });

    describe('numbers', () => {
      it('should convert numbers to NumberAttributeValues', () => {
        expect(new Marshaller().marshallValue(42)).toEqual({ N: '42' });
      });

      it('should convert BigInts to NumberAttributeValues', () => {
        expect(new Marshaller().marshallValue(BigInt('123'))).toEqual({
          N: '123',
        });
      });
    });
    ...
    describe('null', () => {
      it('should convert nulls to NullAttributeValues', () => {
        expect(new Marshaller().marshallValue(null)).toEqual({
          NULL: true,
        });
      });
    });
    ...
    describe('boolean', () => {
      it('should convert booleans to BooleanAttributeValues', () => {
        ...
        expect(marshaller.marshallValue(false)).toEqual({
          BOOL: false,
        });
      });
    });
    ...
    describe('lists', () => {
      ...
      it('should convert list members to AttributeValues', () => {
        expect(new Marshaller().marshallValue(['a', 1, true, null, {}])).toEqual({
          L: [{ S: 'a' }, { N: '1' }, { BOOL: true }, { NULL: true }, { M: {} }],
        });
      });
    ...
    describe('maps', () => {
      ...
      it('should convert maps to MapAttributeValues', () => {
        expect(new Marshaller().marshallValue(new Map())).toEqual({
          M: {},
        });
      });

      it('should omit keys whose values are serialized as undefined', () => {
        const marshaller = new Marshaller();
        expect(marshaller.marshallValue({ a: undefined })).toEqual({
          M: {},
        });

        expect(marshaller.marshallValue(new Map([['a', undefined]]))).toEqual({ M: {} });
      });
      ...
      it.skip('should omit map members whose keys are not strings when the onInvalid option is "omit"', () => {
        ...
        expect(marshaller.marshallValue(map)).toEqual({
          M: { a: { S: 'a' } },
        });
      });
    ...
    describe('sets', () => {
      ...
      it.skip('should omit sets with members of an unknown type when the onEmpty option is "omit"', () => {
        ...
        const set = new Set<Record<string, unknown>>();
        set.add({});
        expect(marshaller.marshallValue(set)).toBeUndefined();
      });

      it('should throw on sets with members of an unknown type otherwise', () => {
        const marshaller = new Marshaller();
        const set = new Set<Record<string, unknown>>();
        set.add({});
      ...
      it.skip('should drop invalid members when onInvalid option is set to "omit"', () => {
        ...
        expect(marshaller.marshallValue(new Set(['a', 1, 'c']))).toEqual({ SS: ['a', 'c'] });
      });

      it.skip('should throw on invalid members otherwise', () => {
        const marshaller = new Marshaller();
        expect(() => marshaller.marshallValue(new Set(['a', 1, 'c']))).toThrow();
      });
      ...
      describe('string sets', () => {
        it('should convert sets with strings into StringSetAttributeValues', () => {
          expect(new Marshaller().marshallValue(new Set<string>(['a', 'b', 'c']))).toEqual({
            SS: ['a', 'b', 'c'],
          });
        });
        ...
        it.skip('should drop empty members when onEmpty option is set to "nullify"', () => {
          expect(
            new Marshaller({ onEmpty: 'nullify' }).marshallValue(new Set<string>(['a', '', 'c']))
          ).toEqual({ SS: ['a', 'c'] });
        });

        it.skip('should drop empty members when onEmpty option is set to "omit"', () => {
          expect(
            new Marshaller({ onEmpty: 'omit' }).marshallValue(new Set<string>(['a', '', 'c']))
          ).toEqual({ SS: ['a', 'c'] });
        });

        it('should keep empty members otherwise', () => {
          expect(new Marshaller().marshallValue(new Set<string>(['a', '', 'c']))).toEqual({
            SS: ['a', '', 'c'],
          });
        });
      });
      ...
      describe('number sets', () => {
        it('should convert sets with numbers into NumberSetAttributeValues', () => {
          expect(new Marshaller().marshallValue(new Set<number>([1, 2, 3]))).toEqual({
            NS: ['1', '2', '3'],
          });
        });

        it('should convert BigIntSet objects into NumberSetAttributeValues', () => {
          expect(
            new Marshaller().marshallValue(new Set([BigInt('1'), BigInt('2'), BigInt('3')]))
          ).toEqual({ NS: ['1', '2', '3'] });
        });
      });
      ...
      describe.skip('binary sets', () => {
        ...
        it('should drop empty members when the onEmpty option is set to "nullify"', () => {
          const marshaller = new Marshaller({ onEmpty: 'nullify' });
          const converted = marshaller.marshallValue(
            new BinarySet([
              Uint8Array.from([0xde, 0xad]),
        ...
        it('should drop empty members when the onEmpty option is set to "omit"', () => {
          const marshaller = new Marshaller({ onEmpty: 'omit' });
          const converted = marshaller.marshallValue(
            new BinarySet([
              Uint8Array.from([0xde, 0xad]),
    ...
    describe('undefined values', () => {
      it('should return undefined for undefined', () => {
        expect(new Marshaller().marshallValue(undefined)).toBeUndefined();
      });
    });

    describe('symbols', () => {
      it('should omit symbols when the onInvalid option is set to "omit"', () => {
        expect(
          new Marshaller({ onInvalid: 'omit' }).marshallValue(Symbol.iterator)
        ).toBeUndefined();
      });

      it('should throw on symbols otherwise', () => {
        expect(() => new Marshaller().marshallValue(Symbol.iterator)).toThrow();
      });
    });

    describe('functions', () => {
      it('should omit functions when the onInvalid option is set to "omit"', () => {
        expect(new Marshaller({ onInvalid: 'omit' }).marshallValue(() => {})).toBeUndefined();
      });

      it('should throw on symbols otherwise', () => {
        expect(() => new Marshaller().marshallValue(() => {})).toThrow();
      });
    });
  });

  describe('#unmarshallItem', () => {
    it('should convert DynamoDB items to plain vanilla JS objects', () => {
      let unmarshalled = new Marshaller({
        unwrapNumbers: true,
      }).unmarshallItem({
        string: { S: 'foo' },
        list: { L: [{ S: 'fizz' }, { S: 'buzz' }, { S: 'pop' }] },
  ...
  describe('#unmarshallValue', () => {
    ...
    describe('strings', () => {
      it('should convert StringAttributeValues to strings', () => {
        expect(marshaller.unmarshallValue({ S: 'string' })).toEqual('string');
      });
    });

    describe('binary values', () => {
      it('should convert BinaryAttributeValues to binary values', () => {
        expect(marshaller.unmarshallValue({ B: new Uint8Array(1) })).toEqual(new Uint8Array(1));
      });
    });

    describe('numbers', () => {
      it('should convert NumberAttributeValues to BigInts', () => {
        const unsafeInteger = '9007199254740991000';
        const converted = marshaller.unmarshallValue({
          N: unsafeInteger,
        }) as BigInt;
        expect(converted.toString()).toBe(unsafeInteger);
      });

      it('should convert NumberAttributeValues to numbers when unwrapNumbers is true', () => {
    ...
    describe('boolean', () => {
      it('should convert BooleanAttributeValues to booleans', () => {
        ...
        expect(marshaller.unmarshallValue({ BOOL: false })).toEqual(false);
      });
    });
    ...
    describe('lists', () => {
      ...
      it('should convert member AttributeValues to list members', () => {
        expect(
          marshaller.unmarshallValue({
            L: [{ S: 'a' }, { N: '1' }, { BOOL: true }, { NULL: true }, { M: {} }],
          })
        ).toEqual(['a', BigInt('1'), true, null, {}]);
      });
    });
    ...
    describe('string sets', () => {
      it('should convert StringSetAttributeValues into sets with strings', () => {
        expect(marshaller.unmarshallValue({ SS: ['a', 'b', 'c'] })).toEqual(
          new Set(['a', 'b', 'c'])
        );
      });
    });
    ...
    describe('number sets', () => {
      it('should convert NumberSetAttributeValues into sets with BigInts', () => {
        const unsafeInteger = '900719925474099100';
        const converted = marshaller.unmarshallValue({
          NS: [unsafeInteger + '1', unsafeInteger + '2', unsafeInteger + '3'],
        });

        expect(converted).toEqual(
          new Set([
            BigInt(unsafeInteger + '1'),
            BigInt(unsafeInteger + '2'),
            BigInt(unsafeInteger + '3'),
          ])
        );
      });

      it('should convert NumberSetAttributeValues into sets with numbers when unwrapNumbers is true', () => {
        const marshaller = new Marshaller({ unwrapNumbers: true });
        expect(marshaller.unmarshallValue({ NS: ['1', '2', '3'] })).toEqual(new Set([1, 2, 3]));
      });
    });

    describe('binary sets', () => {
      it('should convert BinarySetAttributeValues into sets with binary strings', () => {
        expect(
          marshaller.unmarshallValue({
            BS: [new Uint8Array(1), new Uint8Array(2)],
          })
        ).toEqual(new BinarySet([new Uint8Array(1), new Uint8Array(2)]));
      });
    });

    it('should convert objects with no values to empty maps', () => {
      expect(marshaller.unmarshallValue({ foo: 'bar' } as any)).toEqual({});
    });
  });
});
