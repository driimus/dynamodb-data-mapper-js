...
import {SchemaType, TypeTag} from './SchemaType';

const keyableTypes: TypeTag[] = [
	'Binary',
	'Custom',
	'Date',
	'Number',
...
describe('isKey', () => {
	it.each(unkeyableTypes)('should return false if the field is of type $notKeyType', notKeyType => {
		expect(
			isKey({type: notKeyType, keyType: 'HASH'} as SchemaType),
		).toBe(false);
	});

	it.each(keyableTypes)('should return false if the field is of type $keyType', keyType => {
		expect(
			isKey({type: keyType, keyType: 'HASH'} as SchemaType),
		).toBe(true);
	});
	...
	it('should return true if the field is an index key and the index name was supplied', () => {
		expect(
			isKey(
				{type: 'String', indexKeyConfigurations: {foo: 'HASH'}},
				'foo',
			),
		).toBe(true);
	});
	...
	it('should return false if the field is an index key and no index name was supplied', () => {
		expect(
			isKey({type: 'String', indexKeyConfigurations: {foo: 'HASH'}}),
		).toBe(false);
	});
