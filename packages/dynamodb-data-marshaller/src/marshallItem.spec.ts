...
describe('marshallItem', () => {
    ...
    it('should ignore fields not mentioned in the schema', () => {
        expect(
            marshallItem({ foo: { type: 'String' } }, { bar: 'baz' })
        ).toEqual({});
    });

    it('should ignore fields whose value is undefined', () => {
        expect(
            marshallItem({ foo: { type: 'String' } }, { foo: void 0 })
        ).toEqual({});
    });

    it('should throw if the schema type tag is not recognized', () => {
        expect(() =>
            marshallItem({ foo: { type: 'Foo' } } as any, { foo: 'bar' })
    ...
    describe.skip('binary set fields', () => {
        ...
        it('should deduplicate values included in the input', () => {
            expect(
                marshallItem(schema, {
                    binSet: new Set([
                        Uint8Array.from([
                            240, 159, 144, 142, 240, 159, 145, 177, 226, 157,
                            164,
                        ]).buffer,
                        Uint8Array.from([
                            240, 159, 144, 142, 240, 159, 145, 177, 226, 157,
        ...
        it('should render empty sets as NullAttributeValues', () => {
            expect(
                marshallItem(schema, { binSet: [new ArrayBuffer(0)] })
            ).toEqual({
                binSet: { NULL: true },
            });
        });
    ...
    describe('date fields', () => {
        ...
        it('should throw if an unexpected input is received', () => {
            ...
            expect(() =>
                marshallItem(schema, { aDate: new ArrayBuffer(10) })
            ).toThrow(
    ...
    describe('map fields', () => {
        ...
        it('should serialize an object with like values', () => {
            expect(
                marshallItem(schema, { map: { foo: 'bar', fizz: 'buzz' } })
            ).toEqual({
                map: {
                    M: {
                        foo: { S: 'bar' },
    ...
    describe('null fields', () => {
        it('should always return a null AttributeValue', () => {
            for (let value of [
                ...
                expect(
                    marshallItem({ value: { type: 'Null' } }, { value })
                ).toEqual({
                    value: { NULL: true },
                });
            }
    ...
    describe('number fields', () => {
        it('should marshall number fields', () => {
            expect(
                marshallItem({ num: { type: 'Number' } }, { num: 123 })
            ).toEqual({
                num: { N: '123' },
            });
        });
    ...
    describe('number set fields', () => {
        ...
        it('should serialize NumberSet fields', () => {
            expect(
                marshallItem(schema, { numSet: new Set([1, 2, 3]) })
            ).toEqual({
                numSet: { NS: ['1', '2', '3'] },
            });
        });
        ...
        it('should deduplicate values included in the input', () => {
            expect(
                marshallItem(schema, { numSet: new Set([1, 2, 3, 1]) })
            ).toEqual({
                numSet: { NS: ['1', '2', '3'] },
            });
        });
    ...
    describe('set fields', () => {
        ...
        it('should throw an error if the memberType is not recognized', () => {
            expect(() =>
                marshallItem(schema, { fooSet: [1, 2, 3, 1] })
    ...
    describe('string fields', () => {
        it('should marshall string fields', () => {
            expect(
                marshallItem({ str: { type: 'String' } }, { str: 'string' })
            ).toEqual({ str: { S: 'string' } });
        });

        it('should marshall stringable objects', () => {
            expect(
                marshallItem({ str: { type: 'String' } }, { str: {} })
            ).toEqual({
                str: { S: '[object Object]' },
            });
        });
    ...
    describe('string set fields', () => {
        ...
        it('should serialize StringSet fields', () => {
            expect(
                marshallItem(schema, { strSet: new Set(['a', 'b', 'c']) })
            ).toEqual({
                strSet: { SS: ['a', 'b', 'c'] },
            });
        });
        ...
        it('should deduplicate values included in the input', () => {
            expect(
                marshallItem(schema, { strSet: ['a', 'b', 'c', 'a'] })
            ).toEqual({
                strSet: { SS: ['a', 'b', 'c'] },
            });
        });
        ...
        it.skip('should remove undefined values from sets', () => {
            expect(
                marshallItem(schema, { strSet: ['', 'a', 'b', 'c', ''] })
            ).toEqual({
                strSet: { SS: ['a', 'b', 'c'] },
            });
        });
