...
import { unmarshallItem } from '../src/unmarshallItem';
...
describe('unmarshallItem', () => {
  ...
  it('should throw if the schema type tag is not recognized', () => {
    const schema: Schema = {
      binary: { type: 'Foo' as 'Boolean' },
    };

    expect(() => unmarshallItem(schema, { binary: { S: 'a string' } })).toThrow();
  });
  ...
  describe('binary fields', () => {
    ...
    it('should unmarshall binary fields', () => {
      expect(unmarshallItem(schema, { binary: { B: new Uint8Array(15) } })).toEqual({
        binary: new Uint8Array(15),
      });
    });

    it('should convert null values to an empty binary value', () => {
      expect(unmarshallItem(schema, { binary: { NULL: true } })).toEqual({
    ...
    it('should marshall of untyped data', () => {
      ...
      const input = {
        mixedList: {
          L: [
            ...
            { M: { foo: { S: 'bar' } } },
            {
              L: [{ S: 'one string' }, { N: '234' }, { B: new ArrayBuffer(5) }],
            },
          ],
        },
      };
      ...
      expect(unmarshallItem(schema, input as any)).toEqual({
        mixedList: [
          ...
          new ArrayBuffer(12),
          { foo: 'bar' },
          ['one string', BigInt(234), new ArrayBuffer(5)],
        ],
      });
    });
  ...
  describe('binary set fields', () => {
    ...
    it('should unmarshall binary set fields', () => {
      const attrMap = {
        binSet: {
          BS: [new Uint8Array(1), new Uint8Array(2), new Uint8Array(3)],
        },
      };
    ...
    it('should unmarshall null values as empty binary sets', () => {
      expect(unmarshallItem(schema, { binSet: { NULL: true } })).toEqual({
        binSet: new BinarySet(),
      });
    });

    it('should unmarshall type mismatches as undefined', () => {
      expect(unmarshallItem(schema, { binSet: { BOOL: true } })).toEqual({
        binSet: undefined,
      });
    });
  });
  ...
  describe('boolean fields', () => {
    ...
    it('should unmarshall boolean fields', () => {
      expect(unmarshallItem(schema, { boolean: { BOOL: false } })).toEqual({ boolean: false });
    });
  });
    ...
    it('should unmarshall untyped collections', () => {
      ...
      const input = {
        mixedList: {
          L: [
            ...
            { M: { foo: { S: 'bar' } } },
            {
              L: [{ S: 'one string' }, { N: '234' }, { B: new Uint8Array(5) }],
            },
          ],
        },
      };
      ...
      expect(unmarshallItem(schema, input)).toEqual({
        mixedList: [
          ...
          new Uint8Array(12),
          { foo: 'bar' },
          ['one string', BigInt('234'), new Uint8Array(5)],
        ],
      });
    });
  });
  ...
  describe('custom fields', () => {
    it('should unmarshall custom fields by invoking the unmarshaller defined in the schema', () => {
      const unmarshall = vitest.fn(() => 'unmarshalled');
      const schema: Schema = {
        custom: {
          type: 'Custom',
          marshall: vitest.fn(),
      ...
      expect(unmarshallItem(schema, { custom: { NULL: true } })).toEqual({
        custom: 'unmarshalled',
      });
  ...
  describe('date fields', () => {
    ...
    const epoch = 946_684_800;

    it('should unmarshall dates into Date objects', () => {
      expect(unmarshallItem(schema, { aDate: { N: epoch.toString(10) } })).toEqual({
        aDate: new Date(iso8601),
      });
    });

    it('should leaves dates undefined if the value at the designated key is not a number', () => {
      expect(unmarshallItem(schema, { aDate: { S: epoch.toString(10) } })).toEqual({});
    });
  });
  ...
  describe('document fields', () => {
    it('should recursively unmarshall documents', () => {
      ...
      expect(unmarshallItem(schema, input)).toEqual({
        nested: { nested: { scalar: 'value' } },
      });
    });

    it('should invoke the constructor defined in the schema for documents', () => {
      ...
      const unmarshalled = unmarshallItem(schema, { ctorDoc: { M: {} } });
    ...
    it('should unmarshall untyped hashes', () => {
      ...
      expect(unmarshallItem(schema, input)).toEqual({
        mixedHash: {
          ...
          quux: new BinarySet([new Uint8Array(1), new Uint8Array(2), new Uint8Array(3)]),
          fizz: { foo: 'bar' },
          buzz: ['one string', BigInt('234'), new Uint8Array(5)],
          pop: new Set([BigInt('123'), BigInt('234'), BigInt('345')]),
        },
      });
    });
  });
  ...
  describe('null fields', () => {
    ...
    it('should unmarshall null fields', () => {
      expect(unmarshallItem(schema, { null: { NULL: true } })).toEqual({
        null: null,
      });
    });
  ...
  describe('number fields', () => {
    ...
    it('should unmarshall number fields', () => {
      expect(unmarshallItem(schema, { number: { N: '123' } })).toEqual({
        number: 123,
      });
    });
  ...
  describe('number set fields', () => {
    ...
    it('should unmarshall number set fields', () => {
      expect(unmarshallItem(schema, { numSet: { NS: ['1', '2', '3'] } })).toEqual({
        numSet: new Set([1, 2, 3]),
      });
    });

    it('should unmarshall null values as empty sets', () => {
      expect(unmarshallItem(schema, { numSet: { NULL: true } })).toEqual({
        numSet: new Set(),
      });
    });

    it('should unmarshall unexpected types as undefined', () => {
      expect(unmarshallItem(schema, { numSet: { SS: ['1', '2', '3'] } })).toEqual({});
    });
  });

  describe('set fields', () => {
    const schema: Schema = {
      fooSet: { type: 'Set', memberType: 'foo' as 'String' },
    };

    it('should throw an error if the memberType is not recognized', () => {
      expect(() => unmarshallItem(schema, { fooSet: { NS: ['1', '2', '3'] } })).toThrowError(
  ...
  describe('string fields', () => {
    ...
    it('should unmarshall string fields', () => {
      expect(unmarshallItem(schema, { string: { S: 'string' } })).toEqual({ string: 'string' });
    });

    it('should unmarshall null values as empty strings', () => {
      expect(unmarshallItem(schema, { string: { NULL: true } })).toEqual({
        string: '',
      });
    });
  });
  ...
  describe('string set fields', () => {
    ...
    it('should unmarshall string set fields', () => {
      expect(unmarshallItem(schema, { strSet: { SS: ['a', 'b', 'c'] } })).toEqual({
        strSet: new Set(['a', 'b', 'c']),
      });
    });

    it('should unmarshall null values as empty sets', () => {
      expect(unmarshallItem(schema, { strSet: { NULL: true } })).toEqual({
        strSet: new Set(),
      });
    });

    it('should unmarshall unexpected types as undefined', () => {
      expect(unmarshallItem(schema, { strSet: { NS: ['a', 'b', 'c'] } })).toEqual({});
    });
  });
  ...
  describe('tuple fields', () => {
    ...
    it('should unmarshall unexpected types as undefined', () => {
      expect(unmarshallItem(schema, { jobResult: { BOOL: true } })).toEqual({});
    });
  });
});
