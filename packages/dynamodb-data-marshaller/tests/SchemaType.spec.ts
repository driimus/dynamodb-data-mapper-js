import { isSchemaType, Schema } from '../src/SchemaType';

describe('isSchemaType', () => {
  it('should reject scalar values', () => {
    for (const scalar of ['string', 123, true, null, undefined]) {
  ...
  describe('keyable types', () => {
    for (const dataType of ['Binary', 'Date', 'Number', 'String']) {
      it(`should accept ${dataType} type declarations with a keyType`, () => {
        expect(
          isSchemaType({
            type: dataType,
            keyType: 'HASH',
      ...
      it(`should reject ${dataType} type declarations with an unrecognized keyType`, () => {
        expect(
          isSchemaType({
            type: dataType,
            keyType: 'foo',
      ...
      it(`should accept ${dataType} type declarations with a hash of indexKeyConfigurations`, () => {
        expect(
          isSchemaType({
            type: dataType,
      ...
      it(`should reject ${dataType} type declarations with a hash of invalid indexKeyConfigurations`, () => {
        expect(
          isSchemaType({
            type: dataType,
      ...
      it(`should reject ${dataType} type declarations with scalar indexKeyConfiguration`, () => {
  ...
  describe('Custom types', () => {
    it('should accept Custom types with a defined marshaller and unmarshaller', () => {
      expect(
        isSchemaType({
          type: 'Custom',
          marshall: () => {},
  ...
  describe('Document types', () => {
    ...
    it('should reject Document types with a non-function valueConstructor', () => {
      expect(
        isSchemaType({
          type: 'Document',
          members: {},
  ...
  describe('Set types', () => {
    it('should accept StringSet types', () => {
      expect(isSchemaType({ type: 'Set', memberType: 'String' })).toBe(true);
    });

    it('should accept NumberSet types', () => {
      expect(isSchemaType({ type: 'Set', memberType: 'Number' })).toBe(true);
    });

    it('should accept BinarySet types', () => {
      expect(isSchemaType({ type: 'Set', memberType: 'Binary' })).toBe(true);
    });
  });
