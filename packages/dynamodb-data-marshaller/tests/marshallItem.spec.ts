...
import { marshallItem } from '../src/marshallItem';
import { CustomType, Schema } from '../src/SchemaType';
...
describe('marshallItem', () => {
  ...
  it('should ignore fields whose value is undefined', () => {
    expect(marshallItem({ foo: { type: 'String' } }, { foo: undefined })).toEqual({});
  });

  it('should throw if the schema type tag is not recognized', () => {
    expect(() => marshallItem({ foo: { type: 'Foo' } } as any, { foo: 'bar' })).toThrow(
      'Unrecognized schema node'
    );
  });

  describe('default values', () => {
    it('should call a defined default provider if the input is undefined', () => {
      const defaultProvider = vitest.fn(() => 'foo');
      expect(
        marshallItem({ foo: { type: 'String', defaultProvider } }, { foo: undefined })
      ).toEqual({ foo: { S: 'foo' } });
    ...
    it('should not call the default provider if the input is defined', () => {
      const defaultProvider = vitest.fn(() => 'foo');
      expect(marshallItem({ foo: { type: 'String', defaultProvider } }, { foo: 'bar' })).toEqual({
        foo: { S: 'bar' },
      });
  ...
  describe('binary fields', () => {
    ...
    it('should convert UTF-8 strings to Uint8Arrays', () => {
      ...
      expect(marshallItem(binaryDoc, document)).toEqual({
        binary: {
          B: new Uint8Array([226, 152, 131, 240, 159, 146, 169]),
        },
      });
    });
  ...
  describe('binary set fields', () => {
    ...
    it('should serialize BinarySet fields', () => {
      expect(
        marshallItem(schema, {
          binSet: new Set([new Uint8Array(1), new Uint8Array(2), new Uint8Array(3)]),
        })
      ).toEqual({
        binSet: {
    ...
    it('should deduplicate values included in the input', () => {
      expect(
        ...
        binSet: {
          BS: [Uint8Array.from([240, 159, 144, 142, 240, 159, 145, 177, 226, 157, 164])],
        },
      });
    });
    ...
    it('should render empty sets as NullAttributeValues', () => {
      expect(marshallItem(schema, { binSet: [new ArrayBuffer(0)] })).toEqual({
        binSet: { NULL: true },
      });
    });
  });
  ...
  describe('custom fields', () => {
    it('should use the marshaller function embedded in the type', () => {
      const marshaller = vitest.fn(() => ({ S: 'stubbed' }));
      const schema = {
        custom: {
          type: 'Custom',
          marshall: marshaller,
          unmarshall: vitest.fn(),
        } as CustomType<void>,
      };
  ...
  describe('date fields', () => {
    ...
    it('should throw if an unexpected input is received', () => {
      ...
      expect(() => marshallItem(schema, { aDate: new ArrayBuffer(10) })).toThrow(
        expect.objectContaining({ invalidValue: new ArrayBuffer(10) })
      );
    });
  });
    ...
    it('should marshall objects of untyped data', () => {
      ...
      const input = {
        mixedObject: {
          ...
          pop: new Set([new Uint8Array(1), new Uint8Array(2), new Uint8Array(3)]),
        },
      };
  ...
  describe('map fields', () => {
    ...
    it('should serialize an object with like values', () => {
      expect(marshallItem(schema, { map: { foo: 'bar', fizz: 'buzz' } })).toEqual({
        map: {
          M: {
            foo: { S: 'bar' },
            fizz: { S: 'buzz' },
  ...
  describe('null fields', () => {
    it('should always return a null AttributeValue', () => {
      for (const value of ['string', 234, false, [], {}, new Int8Array(0)]) {
        expect(marshallItem({ value: { type: 'Null' } }, { value })).toEqual({
          value: { NULL: true },
        });
      }
    });
  ...
  describe('number fields', () => {
    it('should marshall number fields', () => {
      expect(marshallItem({ num: { type: 'Number' } }, { num: 123 })).toEqual({
        num: { N: '123' },
      });
    });
  });
  ...
  describe('number set fields', () => {
    ...
    it('should serialize NumberSet fields', () => {
      expect(marshallItem(schema, { numSet: new Set([1, 2, 3]) })).toEqual({
        numSet: { NS: ['1', '2', '3'] },
      });
    });

    it('should deduplicate values included in the input', () => {
      expect(marshallItem(schema, { numSet: new Set([1, 2, 3, 1]) })).toEqual({
        numSet: { NS: ['1', '2', '3'] },
      });
    });

    it('should render empty sets as NullAttributeValues', () => {
      expect(marshallItem(schema, { numSet: new Set([]) })).toEqual({
        numSet: { NULL: true },
      });
    });
  });
  ...
  describe('set fields', () => {
    ...
    it('should throw an error if the memberType is not recognized', () => {
      expect(() => marshallItem(schema, { fooSet: [1, 2, 3, 1] })).toThrowError(
  ...
  describe('string fields', () => {
    it('should marshall string fields', () => {
      expect(marshallItem({ str: { type: 'String' } }, { str: 'string' })).toEqual({
        str: { S: 'string' },
      });
    });

    it('should marshall stringable objects', () => {
      expect(marshallItem({ str: { type: 'String' } }, { str: {} })).toEqual({
        str: { S: '[object Object]' },
      });
    });
  ...
  describe('string set fields', () => {
    ...
    it('should serialize StringSet fields', () => {
      expect(marshallItem(schema, { strSet: new Set(['a', 'b', 'c']) })).toEqual({
        strSet: { SS: ['a', 'b', 'c'] },
      });
    });

    it('should deduplicate values included in the input', () => {
      expect(marshallItem(schema, { strSet: ['a', 'b', 'c', 'a'] })).toEqual({
        strSet: { SS: ['a', 'b', 'c'] },
      });
    });
    ...
    it('should render empty sets as NullAttributeValues', () => {
      expect(marshallItem(schema, { strSet: [] })).toEqual({
        strSet: { NULL: true },
      });
    });
  });
