...
import { DataMapper, DynamoDbSchema } from '@driimus/dynamodb-data-mapper';
import { isSchema } from '@driimus/dynamodb-data-marshaller';
...
describe('annotations', () => {
  ...
  it('should marshall a full object graph according to the schema', async () => {
    const promiseFunc = vitest.fn(async () => Promise.resolve({ Item: {} }));
    // Const mockDynamoDbClient = {
    //     config: {},
    //     putItem: vitest.fn(() => ({promise: promiseFunc})),
    // };
    ...
    reply.timestamp = new Date(60_000);
