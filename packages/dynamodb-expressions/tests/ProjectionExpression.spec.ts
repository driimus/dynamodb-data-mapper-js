...
import { serializeProjectionExpression } from '../src/ProjectionExpression';

describe('ProjectionExpression', () => {
  it('should allow the addition of scalar values', () => {
    const attributes = new ExpressionAttributes();

    expect(serializeProjectionExpression(['foo', 'bar', 'baz', 'quux'], attributes)).toBe(
  ...
  it('should allow the addition of list index dereferences', () => {
    ...
    expect(serializeProjectionExpression([new AttributePath('foo[2]')], attributes)).toBe(
      '#attr0[2]'
    );
    expect(attributes.names).toEqual({
      '#attr0': 'foo',
  ...
  it('should allow the addition of nested attributes', () => {
    ...
    expect(serializeProjectionExpression([new AttributePath('foo.bar')], attributes)).toBe(
      '#attr0.#attr1'
    );
    expect(attributes.names).toEqual({
      '#attr0': 'foo',
