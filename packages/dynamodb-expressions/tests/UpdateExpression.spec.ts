...
import { UpdateExpression } from '../src/UpdateExpression';
...
describe('UpdateExpression', () => {
  it('should serialize ADD clauses', () => {
    ...
    expect(expr.serialize(attributes)).toBe('ADD #attr0 :val1, #attr2 :val3');
  ...
  it('should serialize DELETE clauses', () => {
    ...
    expect(expr.serialize(attributes)).toBe('DELETE #attr0 :val1, #attr2 :val3');
    expect(attributes.names).toEqual({
      '#attr0': 'foo',
      '#attr2': 'fizz',
    });
  ...
  it('should serialize SET clauses', () => {
    ...
    expect(expr.serialize(attributes)).toBe('SET #attr0 = :val1, #attr2 = :val3');
    expect(attributes.names).toEqual({
      '#attr0': 'foo',
      '#attr2': 'fizz',
    });
  ...
  it('should serialize SET clauses with function expressions', () => {
    const attributes = new ExpressionAttributes();
    const expr = new UpdateExpression();
    expr.set('foo', new FunctionExpression('list_append', new AttributePath('foo'), 'bar'));

    expect(expr.serialize(attributes)).toBe('SET #attr0 = list_append(#attr0, :val1)');
    expect(attributes.names).toEqual({
      '#attr0': 'foo',
    });
    expect(marshall(attributes.values)).toEqual({
  ...
  it('should serialize SET clauses with mathematical expressions', () => {
    const attributes = new ExpressionAttributes();
    const expr = new UpdateExpression();
    expr.set('foo', new MathematicalExpression(new AttributePath('foo'), '+', 1));
