 ...
 */
export interface InequalityExpressionPredicate
	extends BinaryComparisonPredicate {
	type: 'NotEquals';
}
 ...
 */
export interface GreaterThanExpressionPredicate
	extends BinaryComparisonPredicate {
	type: 'GreaterThan';
}
 ...
 * to the object.
 */
export interface GreaterThanOrEqualToExpressionPredicate
	extends BinaryComparisonPredicate {
...
export interface AttributeExistsPredicate
	extends BaseFunctionExpressionPredicate {
...
export function isConditionExpressionPredicate(
	...
	if (arg && typeof arg === 'object') {
		switch (arg.type) {
			...
			case 'Between':
				return (
					arg.lowerBound !== undefined && arg.upperBound !== undefined
				);
			case 'Membership':
...
export function isConditionExpressionSubject(
	...
	return (
        ...
        && (typeof arg.subject === 'string'
            || AttributePath.isAttributePath(arg.subject))
	);
}
...
export function isConditionExpression(arg: any): arg is ConditionExpression {
	...
	if (Boolean(arg) && typeof arg === 'object') {
		switch (arg.type) {
			...
			case 'And':
			case 'Or':
				return Array.isArray(arg.conditions) ? (arg.conditions as unknown[]).every(c => isConditionExpression(c)) : false;
			default:
				return (
					isConditionExpressionSubject(arg)
...
export function serializeConditionExpression(
	...
	switch (condition.type) {
		...
		case 'Not':
			return `NOT (${serializeConditionExpression(
				condition.condition,
				attributes,
			)})`;
		case 'And':
		case 'Or':
			...
			return condition.conditions
				.map(
					cond =>
						`(${serializeConditionExpression(cond, attributes)})`,
				)
...
function serializeFunctionConditionExpression(condition: (ConditionExpressionSubject & AttributeExistsPredicate) | (ConditionExpressionSubject & AttributeNotExistsPredicate) | (ConditionExpressionSubject & AttributeTypePredicate) | (ConditionExpressionSubject & BeginsWithPredicate) | (ConditionExpressionSubject & ContainsPredicate), subject: AttributePath, attributes: ExpressionAttributes) {
...
function serializeBinaryComparison(
	...
	return `${attributes.addName(
		cond.subject,
	)} ${comparator} ${serializeOperand(cond.object, attributes)}`;
}
