...
function parsePath(path: string): PathElement[] {
	...
	for (
		...
		if (state === ParseState.identifier) {
			({curr, state, collected, peek} = parseIdentifier(curr, state, collected, path, elements, peek, iter));
...
const parseIdentifier = (curr: IteratorResult<string>, state: ParseState, collected: string, path: string, elements: PathElement[], peek: IteratorResult<string>, iter: IterableIterator<string>) => {
	switch (curr.value) {
		case LEFT_BRACKET:
...
const parseListIndex = (curr: IteratorYieldResult<string>, collected: string, path: string, elements: PathElement[], state: ParseState) => {
	switch (curr.value) {
		case RIGHT_BRACKET: {
...
const parseControlCharacter = (curr: IteratorYieldResult<string>, state: ParseState, path: string) => {
	switch (curr.value) {
		case LEFT_BRACKET:
			state = ParseState.listIndex;
			break;
