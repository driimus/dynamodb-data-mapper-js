...
import { Schema } from '@driimus/dynamodb-data-marshaller';
import {
  AttributePath,
  between,
  equals,
...
import { BatchGetOptions, GlobalSecondaryIndexOptions, ParallelScanState } from '../src';
...
describe('DataMapper', () => {
  it('should set the customUserAgent config property on the client', () => {
    ...
    expect(client.config.customUserAgent).toMatchObject(
      expect.arrayContaining([['dynamodb-data-mapper-js', expect.any(String)]])
    );
  });
  ...
  describe('#batchDelete', () => {
    ...
    it.each([true, false])(
      ...
      async (asyncInput) => {
        ...
        expect(mockDynamoDbClient.commandCalls(BatchWriteItemCommand)).toHaveLength(4);
  ...
  describe('#batchGet', () => {
    ...
    it('should allow setting an overall read consistency', async () => {
      ...
      expect(
        mockDynamoDbClient.commandCalls(BatchGetItemCommand).map((call) => call.args[0].input)
      ).toEqual([
        {
          RequestItems: {
            foo: {
    ...
    describe.each([true, false])('should', (asyncInput) => {
      ...
      it('partition get batches into requests with 100 or fewer items', async () => {
        ...
        for (const response of responses) {
          promiseFunc.mockImplementationOnce(async () => Promise.resolve(response));
        }
  ...
  describe('#batchPut', () => {
    ...
    describe.each([true, false])('should', (asyncInput) => {
      it('partition put batches into requests with 25 or fewer items', async () => {
        ...
        const calls = mockDynamoDbClient.commandCalls(BatchWriteItemCommand);
        expect(calls.length).toBe(4);
        expect(calls.map((call) => call.args[0].input)).toEqual(expected);
      });
      ...
      it('retry unprocessed items', async () => {
        ...
        const callCount = calls
          ...
          .reduce<Record<string, number>>(
            (keyUseCount, { RequestItems: { foo } = { foo: undefined } }) => {
              for (const {
                PutRequest: {
                  Item: {
                    fizz: { N: key },
  ...
  describe('#createGlobalSecondaryIndex', () => {
    const describeTablePromiseFunc = vitest.fn(async () =>
      Promise.resolve({
        Table: { TableStatus: 'ACTIVE' },
      })
    );
    const updateTablePromiseFunc = vitest.fn(async () => Promise.resolve({}));
    ...
    it('should make and send an UpdateTable request', async () => {
      ...
      expect(
        mockDynamoDbClient.commandCalls(UpdateTableCommand).map((call) => call.args[0].input)
      ).toEqual([
        {
          TableName: 'foo',
      ...
      expect(mockDynamoDbClient.commandCalls(DescribeTableCommand)).toHaveLength(1);
  ...
  describe('#createTable', () => {
    const createTablePromiseFunc = vitest.fn(async () => Promise.resolve({}));
    const describeTablePromiseFunc = vitest.fn(async () =>
      Promise.resolve({
        Table: { TableStatus: 'ACTIVE' },
      })
    ...
    it('should make and send a CreateTable request', async () => {
      ...
      expect(
        mockDynamoDbClient.commandCalls(CreateTableCommand).map((call) => call.args[0].input)
      ).toEqual(
        // [
        [
          {
    ...
    it('should forgo invoking the waiter if the table is already active', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(CreateTableCommand).length).toBe(1);

      // Expect(mockDynamoDbClient.waitFor.mock.calls.length).toBe(0);
      expect(mockDynamoDbClient.commandCalls(DescribeTableCommand)).toHaveLength(0);
    });
    ...
    describe('index keys', () => {
      ...
      it('should throw if no options were provided for a modeled index', async () => {
        ...
        await expect(mapper.createTable(IndexedItem, options)).rejects.toMatchObject(
  ...
  describe('#delete', () => {
    const promiseFunc = vitest.fn(async () => Promise.resolve({ Attributes: {} }));
    const mockDynamoDbClient = mockClient(DynamoDBClient);
    ...
    it('should not include a condition expression when the schema contains a version attribute but the value is undefined', async () => {
      ...
      expect(
        mockDynamoDbClient.commandCalls(DeleteItemCommand)[0].args[0].input
    ...
    it('should not include ExpressionAttributeValues when a substitution has not been made', async () => {
      await mapper.delete(
        ...
        {
          condition: new FunctionExpression('attribute_not_exists', new AttributePath('fizz')),
        }
      );

      expect(mockDynamoDbClient.commandCalls(DeleteItemCommand)[0].args[0].input).toEqual({
  ...
  describe('#deleteTable', () => {
    const deleteTablePromiseFunc = vitest.fn(async () => Promise.resolve({}));
    const describeTablePromiseFunc = vitest.fn(async () => Promise.resolve({}));
    ...
    it('should make and send a DeleteTable request and wait for it to take effect', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(DeleteTableCommand)[0].args[0].input).toEqual({
        TableName: 'foo',
      });
  ...
  describe('#ensureGlobalSecondaryIndexExists', () => {
    ...
    mapper.createGlobalSecondaryIndex = vitest.fn(async () => Promise.resolve());

    beforeEach(() => {
      (mapper.createGlobalSecondaryIndex as Mock).mockClear();
      mockDynamoDbClient.resetHistory();
    });
    ...
    it('should resolve immediately if the table exists, is active, and the GSI already exists', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(DescribeTableCommand)[0].args[0].input).toEqual({
        TableName: tableName,
      });
    ...
    it('should rethrow if "describeTable" throws a "ResourceNotFoundException"', async () => {
      ...
      await expect(
        mapper.ensureGlobalSecondaryIndexExists(Item, 'DescriptionIndex', {
          indexOptions: {
            DescriptionIndex,
          },
          readCapacityUnits: 5,
  ...
  describe('#ensureTableExists', () => {
    ...
    mapper.createTable = vitest.fn(async () => Promise.resolve());

    beforeEach(() => {
      (mapper.createTable as Mock).mockClear();
      mockDynamoDbClient.resetHistory();
      describeTablePromiseFunc.mockClear();
    });
    ...
    it('should attempt to create the table if "describeTable" throws a "ResourceNotFoundException"', async () => {
      ...
      expect((mapper.createTable as any).mock.calls).toEqual([[Item, options]]);
    ...
    it('should rethrow any service exception other than "ResourceNotFoundException"', async () => {
      ...
      await expect(mapper.ensureTableExists(Item, options)).rejects.toMatchObject(
        new Error('PANIC')
      );
  ...
  describe('#ensureTableNotExists', () => {
    // Const waitPromiseFunc = vitest.fn(async () =>
    ...
    //     config: {},
    //     describeTable: vitest.fn(() => ({
    //         promise: describeTablePromiseFunc,
    //     })),
    //     waitFor: vitest.fn(() => ({ promise: waitPromiseFunc })),
    // };
    mockDynamoDbClient.on(DescribeTableCommand).callsFake(describeTablePromiseFunc);

    const mapper = new DataMapper({
      client: mockDynamoDbClient as unknown as DynamoDBClient,
    });
    mapper.deleteTable = vitest.fn(async () => Promise.resolve());

    beforeEach(() => {
      (mapper.deleteTable as Mock).mockClear();
      mockDynamoDbClient.resetHistory();
      describeTablePromiseFunc.mockClear();
      // WaitPromiseFunc.mockClear();
    });
    ...
    it('should wait for the table not to exist if its state is "DELETING"', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(DescribeTableCommand).length).toBe(2);
    ...
    it('should wait for the table to exist if its state is "CREATING", then delete it', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(DescribeTableCommand)).toHaveLength(2);
      // Expect((waitUntilTableNotExists as any).mock.calls).toEqual([
    ...
    it('should rethrow any service exception other than "ResourceNotFoundException"', async () => {
      ...
      await expect(mapper.ensureTableNotExists(Item)).rejects.toMatchObject(new Error('PANIC'));
  ...
  describe('#get', () => {
    const promiseFunc = vitest.fn(async () => Promise.resolve({ Item: {} } as GetItemOutput));
    const mockDynamoDbClient = mockClient(DynamoDBClient);
    ...
    it('should use the table name specified in the supplied table definition', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(GetItemCommand)[0].args[0].input).toMatchObject({
        TableName: tableName,
      });
    });
  ...
  describe('#parallelScan', () => {
    ...
    it('should resume from a provided scanState', async () => {
      ...
      expect(
        mockDynamoDbClient.commandCalls(ScanCommand).map(({ args: [{ input }] }) => [input])
      ).toEqual([
        [
          {
            TableName: 'foo',
  ...
  describe('#put', () => {
    const promiseFunc = vitest.fn(async () => Promise.resolve({ Item: {} } as PutItemOutput));
    const mockDynamoDbClient = mockClient(DynamoDBClient);
    ...
    it('should use the table name specified in the supplied table definition', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(PutItemCommand)[0].args[0].input).toMatchObject({
        TableName: tableName,
      });
    });
    ...
    it('should include a condition expression requiring that no versioned item be present when the schema contains a version attribute but the value is undefined', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(PutItemCommand)[0].args[0].input).toEqual({
        Item: {
          foo: { S: 'buzz' },
          pop: { N: '0' },
        },
    ...
    it('should not include a condition expression when the skipVersionCheck input parameter is true', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(PutItemCommand)[0].args[0].input).not.toHaveProperty(
        'ConditionExpression'
      );
    });
    ...
    it('should return the unmarshalled input', async () => {
      promiseFunc.mockImplementation(async () => Promise.resolve({} as PutItemOutput));
  ...
  describe('#query', () => {
    ...
    beforeEach(() => {
      promiseFunc.mockClear();
      promiseFunc.mockImplementation(async () => Promise.resolve({ Attributes: {} }));
      mockDynamoDbClient.resetHistory();
    });
    ...
    it('should request a consistent read if the readConsistency is StronglyConsistent', async () => {
      const results = mapper.query(QueryableItem, { foo: 'bar' }, { readConsistency: 'strong' });

      await results.next();

      expect(mockDynamoDbClient.commandCalls(QueryCommand)[0].args[0].input).toMatchObject({
        ConsistentRead: true,
      });
    });
      ...
      const results = mapper.query(
        ...
        },
        {
          ...
          conditions: [
            ...
            new FunctionExpression('begins_with', new AttributePath('fizz'), 'buz'),
          ],
        }
      );
    ...
    it('should support detaching the paginator', async () => {
      ...
      for await (const page of paginator) {
        expect(page).toEqual([QueryableItem.fromKey('foo'), QueryableItem.fromKey('bar')]);
      }
    ...
    it('should cease iteration once the limit has been reached', async () => {
      promiseFunc.mockImplementationOnce(async () =>
        Promise.resolve({
          Items: [{ snap: { S: 'snap' } }, { snap: { S: 'crackle' } }, { snap: { S: 'pop' } }],
      ...
      expect(results.pages().lastEvaluatedKey).toEqual(QueryableItem.fromKey('buzz'));

      expect(
        mockDynamoDbClient.commandCalls(QueryCommand).map((call) => [call.args[0].input])
      ).toEqual([
        [
          {
            TableName: 'foo',
    ...
    describe('startKey serialization', () => {
      ...
      it('should not inject default values into the startKey', async () => {
        const iter = mapper.query(MyItem, { snap: 'key' }, { startKey: new MyItem('key') });
        await iter.next();

        expect(
          mockDynamoDbClient.commandCalls(QueryCommand)[0].args[0].input.ExclusiveStartKey
        ).toEqual({
          snap: { S: 'key' },
        });
      });
  ...
  describe('#scan', () => {
    ...
    it('should request a consistent read if the readConsistency is StronglyConsistent', async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(ScanCommand)[0].args[0].input).toMatchObject({
        ConsistentRead: true,
      });
    });
    ...
    it('should cease iteration once the limit has been reached', async () => {
      ...
      expect(results.pages().lastEvaluatedKey).toEqual(ScannableItem.fromKey('buzz'));

      expect(
        mockDynamoDbClient.commandCalls(ScanCommand).map((call) => [call.args[0].input])
      ).toEqual([
        [
          {
            TableName: 'foo',
    ...
    describe('startKey serialization', () => {
      ...
      it('should not inject default properties into the startKey', async () => {
        ...
        expect(
          mockDynamoDbClient.commandCalls(ScanCommand)[0].args[0].input.ExclusiveStartKey
        ).toEqual({
          snap: { S: 'key' },
        });
      });
  ...
  describe('updating items', () => {
    ...
    describe('#update', () => {
      ...
      it('should use the table name specified in the supplied table definition', async () => {
        ...
        expect(mockDynamoDbClient.commandCalls(UpdateItemCommand)[0].args[0].input).toMatchObject({
          TableName: tableName,
        });
      });
