...
describe("DataMapper", () => {
  it.skip("should set the customUserAgent config property on the client", () => {
    ...
    expect(client.config.customUserAgent).toMatch("dynamodb-data-mapper-js/");
  });
  ...
  describe("#batchDelete", () => {
    ...
    for (const asyncInput of [true, false]) {
      it("should should partition delete batches into requests with 25 or fewer items", async () => {
        const deletes: Item[] = [];
        const expected: any = [
          [{ RequestItems: { foo: [] } }],
          [{ RequestItems: { foo: [] } }],
      ...
      it("should should retry unprocessed items", async () => {
        ...
        const callCount = calls.reduce<Record<string, number>>(
          (
            keyUseCount,
            {
              args: [
                {
                  input: { RequestItems: { foo } = { foo: undefined } },
                },
              ],
            }
          ) =>
  ...
  describe("#batchGet", () => {
    ...
    it("should allow setting an overall read consistency", async () => {
      ...
      expect(mockDynamoDbClient.commandCalls(BatchGetItemCommand)).toEqual({
        calls: [
          {
            args: [
              {
    ...
    it("should allow specifying per-table projection expressions", async () => {
      ...
      for (const asyncInput of [true, false]) {
        it("should should partition get batches into requests with 100 or fewer items", async () => {
          const gets: Item[] = [];
          const expected: any = [
          ...
          for (let i = 0; i < 325; i++) {
            ...
            expected[Math.floor(i / 100)][0].RequestItems.foo.Keys.push({
              fizz: { N: String(i) },
            });
          }
        ...
        it("should should retry unprocessed items", async () => {
          ...
          for (let i = 0; i < 325; i++) {
            gets.push(new Item(i));
            expected[currentRequest][0].RequestItems.foo.Keys.push({
              fizz: { N: String(i) },
            });
    ...
    describe("#batchPut", () => {
      ...
      for (const asyncInput of [true, false]) {
        it("should should partition put batches into requests with 25 or fewer items", async () => {
          const puts: Item[] = [];
          const expected: any = [
            [{ RequestItems: { foo: [] } }],
            [{ RequestItems: { foo: [] } }],
    ...
    describe.skip("#ensureTableNotExists", () => {
      const waitPromiseFunc = jest.fn(async () => Promise.resolve());
      ...
      it('should wait for the table not to exist if its state is not "DELETING"', async () => {
        describeTablePromiseFunc.mockImplementationOnce(async () =>
          Promise.resolve({
      ...
      it('should rethrow any service exception other than "ResourceNotFoundException"', async () => {
        ...
        await expect(mapper.ensureTableNotExists(Item)).rejects.toMatchObject(
          new Error("PANIC")
        );
    ...
    describe("#get", () => {
      ...
      const mockDynamoDbClient = mockClient(DynamoDBClient);
      mockDynamoDbClient.on(UpdateTableCommand).callsFake(promiseFunc);
          ...
          promiseFunc.mockImplementationOnce(async () =>
            Promise.resolve({
              Items: [
                {
                  fizz: { S: key },
                  bar: { NS: [(++index).toString(10), (++index).toString(10)] },
                  baz: {
                    L: [
                      { BOOL: index % 2 === 0 },
                      { N: (++index).toString(10) },
      ...
      describe("#update", () => {
        ...
        describe("version attributes", () => {
          ...
          it("should inject a conditional expression requiring the absence of the versioning property and set its value to 0 when an object without a value for it is marshalled", async () => {
            ...
            expect(
              mockDynamoDbClient.commandCalls(UpdateItemCommand)[0].args[0]
                .input
            ).toMatchObject({
              TableName: "table",
              Key: {
