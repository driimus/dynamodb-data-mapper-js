...
export class DataMapper {
  ...
  async createGlobalSecondaryIndex(
    ...
    const indexSearch =
      globalSecondaryIndexes === undefined
        ? []
        : globalSecondaryIndexes.filter(function (index) {
  ...
  async delete<T extends StringToAnyObjectMap = StringToAnyObjectMap>(
    ...
    const { Attributes } = await this.client.send(new DeleteItemCommand(req));
    if (Attributes) {
  ...
  async get<T extends StringToAnyObjectMap = StringToAnyObjectMap>(
    ...
    const { Item } = await this.client.send(new GetItemCommand(req));
    if (Item) {
      return unmarshallItem<T>(
        schema,
        Item,
  ...
  parallelScan<T extends StringToAnyObjectMap>(
    ...
    if (typeof segments !== "number") {
      valueConstructor = (ctorOrParams as ParallelScanParameters<T>)
        .valueConstructor;
    ...
    return new ParallelScanIterator(this.client, valueConstructor, segments, {
      readConsistency: this.readConsistency,
      ...options,
  ...
  async put<T extends StringToAnyObjectMap = StringToAnyObjectMap>(
    ...
    await this.client.send(new PutItemCommand(req));
  ...
  query<T extends StringToAnyObjectMap = StringToAnyObjectMap>(
    ...
    let valueConstructor: ZeroArgumentsConstructor<T>;
    if (!keyCondition) {
      valueConstructor = (valueConstructorOrParameters as QueryParameters<T>)
        .valueConstructor;
      ...
      valueConstructor =
        valueConstructorOrParameters as ZeroArgumentsConstructor<T>;
    }
  ...
  >(
    ...
    const rawResponse = await this.client.send(new UpdateItemCommand(req));
