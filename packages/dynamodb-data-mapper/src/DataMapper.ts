...
export class DataMapper {
   ...
   */
  async *batchDelete<T extends StringToAnyObjectMap>(items: SyncOrAsyncIterable<T>) {
  ...
  async *batchGet<T extends StringToAnyObjectMap>(
    ...
    { readConsistency = this.readConsistency, perTableOptions = {} }: BatchGetOptions = {}
  ) {
   ...
   */
  async *batchPut<T extends StringToAnyObjectMap>(items: SyncOrAsyncIterable<T>) {
  ...
  async *batchWrite<T extends StringToAnyObjectMap>(
    ...
    for await (const [tableName, { DeleteRequest, PutRequest }] of batch) {
      const { keyProperties, itemSchemata } = state[tableName];
      const attributes = PutRequest ? PutRequest.Item : DeleteRequest?.Key ?? {};
      const { constructor, schema } = itemSchemata[itemIdentifier(attributes!, keyProperties)];

      yield [PutRequest ? 'put' : 'delete', unmarshallItem(schema, attributes!, constructor)];
    }
  }
   ...
   */
  async createTable(valueConstructor: ZeroArgumentsConstructor<any>, options: CreateTableOptions) {
    ...
    if (TableStatus !== 'ACTIVE') {
      await waitUntilTableExists({ client: this.client, maxWaitTime: 60 }, { TableName });
    }
  }
  ...
  async createGlobalSecondaryIndex(
    ...
    const { TableDescription: { TableStatus } = { TableStatus: 'UPDATING' } } =
      await this.client.send(
  ...
  async delete<T extends StringToAnyObjectMap = StringToAnyObjectMap>(
    ...
    const { Attributes } = await this.client.send(new DeleteItemCommand(request));
    if (Attributes) {
      return unmarshallItem<T>(schema, Attributes, item.constructor as ZeroArgumentsConstructor<T>);
    }
  }
  ...
  async deleteTable(valueConstructor: ZeroArgumentsConstructor<any>) {
    ...
    await waitUntilTableNotExists({ client: this.client, maxWaitTime: 60 }, { TableName });
  }
  ...
  async ensureTableExists(
    ...
    const TableName = this.getTableName(valueConstructor.prototype);
    try {
      const { Table: { TableStatus } = { TableStatus: 'CREATING' } } = await this.client.send(
   ...
   * @param valueConstructor  The constructor used for values in the table.
   */
  async ensureTableNotExists(valueConstructor: ZeroArgumentsConstructor<any>) {
    const TableName = this.getTableName(valueConstructor.prototype);
    try {
      const { Table: { TableStatus: status } = { TableStatus: 'CREATING' } } =
        await this.client.send(new DescribeTableCommand({ TableName }));
  ...
  async get<T extends StringToAnyObjectMap = StringToAnyObjectMap>(
    ...
    if (Item) {
      return unmarshallItem<T>(schema, Item, item.constructor as ZeroArgumentsConstructor<T>);
    }
  ...
  parallelScan<T extends StringToAnyObjectMap>(
    ...
    return new ParallelScanIterator(this.client, ctorOrParameters, segments, {
   ...
   */
  async executeUpdateExpression<T extends StringToAnyObjectMap = StringToAnyObjectMap>(
  ...
  private async doExecuteUpdateExpression<T extends StringToAnyObjectMap = StringToAnyObjectMap>(
    expression: UpdateExpression,
    key: Record<string, any>,
    schema: Schema,
    tableName: string,
    ...
    request.UpdateExpression = marshallUpdateExpression(expression, schema, attributes).expression;
  ...
  private async *mapWriteBatch<T extends StringToAnyObjectMap>(
    ...
    for await (const [type, item] of items) {
      ...
      const { keyProperties, itemSchemata } = state[tableName];
      const attributes = type === 'delete' ? marshallKey(schema, item) : marshallItem(schema, item);
      const marshalled =
        type === 'delete'
...
function attributeDefinitionList(attributes: AttributeTypeMap): AttributeDefinition[] {
...
function convertBatchGetOptions(options: BatchGetTableOptions, itemSchema: Schema): TableOptions {
...
function indexProjection(schema: Schema, projection: SecondaryIndexProjection): Projection {
...
function itemIdentifier(marshalled: AttributeMap, keyProperties: string[]): string {
